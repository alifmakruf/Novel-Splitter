...
export async function translateChapter({ novelId, chapterKey, text, targetLang = "id" }) {
  const cacheKey = `${novelId}:${chapterKey}:${targetLang}`;
  if (sessionCache.has(cacheKey)) return sessionCache.get(cacheKey);

  const res = await fetch("/api/translate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ novelId, chapterKey, text, targetLang }),
  });
